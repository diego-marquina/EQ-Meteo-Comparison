...
entsoe_gen2.drop(columns='ES_Solar', inplace=True)
entsoe_gen2 = pd.concat([entsoe_gen2,ree_pv], axis=1)
...
for key, value in dict_cty.items():
    if key == 'GB':
        ...
        dict_residuals_solar[key] = value.loc[:,[
            ...
            key+'_PV_Total_lfr',
            key+'_PV_Total_lfe',
        ]].sub(value.loc[:,key+'_Solar_entsoe'], axis=0)
        ...
        if key in ['BE','NL','DK']:
            ...
            dict_residuals_onshore[key] = value.loc[:,[
                ...
                key+'_Onshore_wx_lf',
                key+'_Wind_Onshore_lfr',
                key+'_Wind_Onshore_lfe',
            ]].sub(value.loc[:,key+'_Onshore_entsoe'], axis=0)
            ...
            dict_residuals_offshore[key] = value.loc[:,[
                ...
                key+'_Offshore_wx_lf',
                key+'_Wind_Offshore_lfr',
                key+'_Wind_Offshore_lfe',
            ]].sub(value.loc[:,key+'_Offshore_entsoe'], axis=0)
            ...
            dict_residuals_wind[key] = value.loc[:,[
                ...
                key+'_Wind_Total_lfr',
                key+'_Wind_Total_lfe',
            ]].sub(value.loc[:,key+'_Total_Wind_entsoe'], axis=0)
...
#         for col in dict_residuals_demand[key].columns:
#             print(col+(max([len(col) for col in data.columns])-len(col)+10)*'.'+'\t'+'{}'\
...
for key, data in dict_cty.items():
    ...
    # R2
        ...
        for col in dict_residuals_solar[key].columns:
            print(col+(max([len(col) for col in data.columns])-len(col)+10)*'.'+'\t'+'{:.4f}'\
                .format(metrics.r2_score(
                    data.loc[(~data[col].isna())&(~data[key+'_Solar_entsoe'].isna()),col],
                    data.loc[(~data[col].isna())&(~data[key+'_Solar_entsoe'].isna()),key+'_Solar_entsoe']
                )))   
...
for key, data in dict_cty.items():
    if key =='GB':
        ...
        for col in dict_residuals_onshore[key].columns:
            print(col+(max([len(col) for col in data.columns])-len(col)+10)*'.'+'\t'+'{:.4f}'\
                .format(metrics.r2_score(
                    data.loc[(~data[col].isna())&(~data[key+'_Onshore_entsoe'].isna()),col],
                    data.loc[(~data[col].isna())&(~data[key+'_Onshore_entsoe'].isna()),key+'_Onshore_entsoe']
                )))   
...
for key, data in dict_cty.items():
    if key in ['NL', 'DE', 'BE', 'DK']:
        ...
        for col in dict_residuals_offshore[key].columns:
            print(col+(max([len(col) for col in data.columns])-len(col)+10)*'.'+'\t'+'{:.4f}'\
                .format(metrics.r2_score(
                    data.loc[(~data[col].isna())&(~data[key+'_Offshore_entsoe'].isna()),col],
                    data.loc[(~data[col].isna())&(~data[key+'_Offshore_entsoe'].isna()),key+'_Offshore_entsoe']
                )))
    # R2
    else:
        continue
...
for key, data in dict_cty.items():
    if key =='GB':
        ...
        for col in dict_residuals_wind[key].columns:
            print(col+(max([len(col) for col in data.columns])-len(col)+10)*'.'+'\t'+'{:.4f}'\
                .format(metrics.r2_score(
                    data.loc[(~data[col].isna())&(~data[key+'_Total_Wind_entsoe'].isna()),col],
                    data.loc[(~data[col].isna())&(~data[key+'_Total_Wind_entsoe'].isna()),key+'_Total_Wind_entsoe']
                )))   
